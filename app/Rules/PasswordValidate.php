...
class PasswordValidate implements Rule, DataAwareRule, ValidatorAwareRule
{
    use Conditionable;
    ...
    public function passes($attribute, $value)
    {
        ...
        if ($this->mixedCase && ! preg_match('/(\p{Ll}+.*\p{Lu})|(\p{Lu}+.*\p{Ll})/u', $value)) {
            $this->fail('A senha deve conter pelo menos uma letra maiúscula e uma letra minúscula.');
        }

        if ($this->letters && ! preg_match('/\pL/u', $value)) {
            $this->fail('A senha deve conter pelo menos uma letra.');
        }

        if ($this->symbols && ! preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
            $this->fail('A senha deve conter pelo menos um símbolo.');
        }

        if ($this->numbers && ! preg_match('/\pN/u', $value)) {
            $this->fail('A senha deve conter pelo menos um número.');
        }
